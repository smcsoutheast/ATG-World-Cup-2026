...
async function loadFootballDataScores(schedule) {
  ...
  const url = `${baseUrl || "https://api.football-data.org/v4"}/competitions/${encodeURIComponent(comp)}/matches?season=${encodeURIComponent(season)}`;
  const data = await fetchJson(url, { "X-Auth-Token": apiKey });
